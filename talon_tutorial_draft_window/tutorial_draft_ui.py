...
you know I did by fafe can 'popularneighbor which at least on ^fef'
...
def calculate_percent_read():
    """ 
    says where in a tutorial a person currently is, based on the number of lines in the past file, and the number of lines in the future file.   
    """
    ...
    total_number_of_lines = len(past_lines) + len(future_data) + 1 
    number_of_lines_read = len(past_lines) + 1

    percent_string = str(number_of_lines_read) + "/" + str(total_number_of_lines)
...
def tutorial_name(): 
    """
    """
    if os.path.isfile(os.path.join(__location__, "..\\current_tutorial.txt")):
        with open (os.path.join(__location__, "..\\current_tutorial.txt"),'r') as current_tut_in:
    ...
    return save_basename


class DraftManager:
    """
    API to the tutorial_draft window
    """

    def __init__(self):
        self.area = TextArea()
        self.area.title = "Talon Tutorial:  Commands: next line, last line, page view, line view,  tutorial menu, tutorial close"
        self.area.value = ""
    ...
    def set_styling(
        ...
        """
        Allow settings the style of the tutorial_draft window. Will dynamically
    ...
    def show(self, text: Optional[str] = None):
        ...
        """
        self.area.title = str(tutorial_name()) + " " + str(calculate_percent_read()) + " Commands: next line, last line, page view, line view,  tutorial menu, tutorial close" 
